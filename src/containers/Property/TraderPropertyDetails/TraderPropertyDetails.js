...
const TraderPropertyDetails = props => {
    ...
    const rentFlat = () => {
        dispatch( actions.rentFlat(flat.id) );
    }

    const extraMoney = (flat.condition === 3) ? flat.price : 0;
    const upgradePrice = Math.floor( flat.price * 0.3 + flat.condition * flat.price * 0.1 + extraMoney )

    const upgradeFlat = () => {
        dispatch( actions.upgradeFlat(flat.id, upgradePrice) );
    }

    const rentMoney = rentIncome(flat);
    const taxPrice = sellTax(flat.price).taxPrice;
    ...
    return (
        <div className="row">
            ...
            <div className="col-sm-6">
                ...
                { flat.condition < 4 ? <p>Upgrade price: { GetPriceWithSpaces(upgradePrice) }$</p> : null }
                <p>Square: { flat.square }</p>
                <p>Taxes: { sellTax(flat.price).tax }</p>
                <p>Price - taxes: { GetPriceWithSpaces(taxPrice) } $</p>
                <div>
                    <button className={ classes.btnSuccess + " btn btn-success" } onClick={sellFlat}>Sell</button>
                </div>
                { !flat.rented ? <div>
                    <button className={ classes.btnSuccess + " btn btn-success" } onClick={rentFlat}>Rent</button>
                </div> : null }
                { flat.condition < 4 ? <div>
                    <button className={ classes.btnSuccess + " btn btn-success" } disabled={!balance} onClick={upgradeFlat}>Upgrade</button>
                </div> : null }
